...
#include "./scenes/solar_system.inl"
#include "Image.h"
...
void RayTracer::init(int scene_id) {
    ...
    if (scene_id == 1)
        ...
        scene = std::unique_ptr<Scene>(solar_system());
    else {
...
Ray RayTracer::ray_thru_pixel(int i, int j) {
    ...
    float alpha = (2.0f * (i + x) / camera.width - 1.0f) * tan(glm::radians(40.0f) / 2.0f) * aspect_ratio;
    float beta = (1.0f - 2.0f * (j + y) / camera.height) * tan(glm::radians(40.0f) / 2.0f);
...
void RayTracer::draw() {
    ...
    if (this->shading_mode == ShadingMode::NORMAL) {
         ...
         */
        active_samples_per_pixel = 100;  // TODO: Hardcode this value to 1 once Task 3 is complete
        active_max_bounces = 1;
    } else {
        active_samples_per_pixel = 100;
